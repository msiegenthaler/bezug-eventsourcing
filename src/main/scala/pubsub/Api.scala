...
object Producer {
  /** Publishes data on the topics. After successful (persistent) completion, it replies with 'onPublished'.
    * Messages with the same orderGroup will be delivered to consumers in order. */
  case class Publish(to: Set[Topic], orderGroup: String, data: Any, onPublished: Any) extends Command
}
